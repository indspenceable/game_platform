class DungeonClash
  ...
  def initialize players, args = nil
    @map = Array.new(10){Array.new(10){rand(2)==0 ? 'empty' : 'block'}}
    #@units = [Imp.new(0,'danny',[0,0]), Ent.new(0,'bob',[3,2])], #CHARACTERS
    @players = players 
    @units = [
      {'class'=>'imp','owner'=>@players[0],'location'=>[0,0]},
      {'class'=>'ent','owner'=>@players[1],'location'=>[3,2]}
    ]
  end
